...
|----------------------|----------------------|
| [fsm_pid_llm_sim.py](https://samizo-aitl.github.io/EduController/part09_llm_hybrid/simulation/fsm_pid_llm_sim.py) | 三層制御統合シミュレーション（FSM × PID × LLM）<br>*Integrated simulation of FSM × PID × LLM* |
| [goal_reasoning_agent.py](https://samizo-aitl.github.io/EduController/part09_llm_hybrid/simulation/goal_reasoning_agent.py) | LLMベースの目的推論エージェントクラス<br>*LLM-based goal reasoning agent* |
| [hybrid_control_demo.ipynb](https://samizo-aitl.github.io/EduController/part09_llm_hybrid/notebooks/hybrid_control_demo.ipynb) | 統合制御のNotebook可視化デモ（予定）<br>*Notebook demo for integrated control (planned)* |
...
|------------------|------------------|-----------------------------|------------------|
| 第1章 | FSM・PID・LLM統合  <br>*FSM, PID, and LLM Integration* | `fsm_pid_llm_sim.py` | 統合制御PoC、状態遷移＋PID＋LLM <br>*Integrated control PoC, state transitions + PID + LLM* |
| 第2章 | シナリオ制御 <br>*Scenario Control* | `fsm_pid_llm_sim.py` | FSMに基づくモード管理 <br>*FSM-based mode management* |
| 第3章 | 例外処理 <br>*Exception Handling* | LLMログ出力部 | 状況判断・切替ロジック含む <br>*Includes context judgment and transition logic* |
...
- **Attentionの役割 / Role of Attention**  
  過去系列の情報を動的に重み付けして利用 → 制御では「PIDの固定ゲイン」と対比し、**可変的な重み付け**が可能  
